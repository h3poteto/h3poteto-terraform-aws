data "template_file" "web_container_definition" {
  ...
  vars {
    phoenix_container_image = "${data.terraform_remote_state.aws_ecr_tokyo.pleroma_phoenix_url}:latest"
    container_image         = "${data.terraform_remote_state.aws_ecr_tokyo.pleroma_nginx_url}:latest"
